...
SELECT COUNT(c.nid) FROM node n INNER JOIN comments c ON n.nid = c.nid WHERE n.nid = 0 AND n.status = 1 AND c.status = 0 AND timestamp > '$history' GROUP BY n.nid;
...
SELECT MAX(thread) FROM comments WHERE thread = '%s.%%' AND nid = '%d';
...
SELECT COUNT(url) FROM accesslog WHERE url <> '' AND url <> '%check_query($_SERVER[HTTP_HOST])%';
SELECT c.cid as cid, c.pid, c.nid, c.subject, c.comment, c.timestamp, u.uid, u.name, u.data, c.score, c.users, c.thread FROM comments c INNER JOIN users u ON c.uid = u.uid WHERE c.nid = 'check_query($nid)' AND c.status = 0;
...
SELECT * FROM item WHERE fid = 0 ORDER BY iid DESC;
SELECT * FROM site WHERE changed > '(time() - 604800) ' ORDER BY changed DESC;
SELECT * FROM users WHERE name = '% check_query($keys) %';
SELECT i.*, f.title AS feed FROM item i INNER JOIN feed f ON i.fid = f.fid ORDER BY i.iid DESC;
SELECT i.*, f.title AS ftitle, f.link AS flink FROM item i INNER JOIN feed f ON i.fid = f.fid ORDER BY i.iid DESC;
...
-- SELECT n.*, u.name FROM node n INNER JOIN users u ON n.uid = u.uid WHERE n.type = 'blog' AND n.uid = 0 ORDER BY n.nid DESC;
SELECT ov.*, ov.name FROM ourViewN ov WHERE ov.type = 'blog' AND ov.uid = 0 ORDER BY ov.nid DESC;
...
SELECT u.*, s.*, r.name AS role FROM users u INNER JOIN role r ON u.rid = r.rid INNER JOIN sessions s ON u.uid = s.uid WHERE s.sid = 'check_query($key)' AND u.status < 3;
...
SELECT n.nid, n.title, n.created, MAX(c.timestamp) AS date_sort, COUNT(c.nid) AS num_comments FROM node n INNER JOIN forum f ON n.nid = f.nid INNER JOIN comments c ON n.nid = c.nid WHERE n.nid = f.nid AND f.tid = 0 AND n.status = 1 GROUP BY n.nid, n.title, n.created ORDER BY '$user->sortby';
...
-- SELECT n.*, u.name, u.uid FROM node n INNER JOIN users u ON n.uid = u.uid WHERE n.title = '%". check_query($title) ."%' AND n.status = 1 ORDER BY n.created DESC;
SELECT ov.*, ov.name, ov.uid FROM ourViewN ov WHERE ov.title = '%". check_query($title) ."%' AND ov.status = 1 ORDER BY ov.created DESC;
...
SELECT s.nid, n.title, u.uid, u.name FROM node_counter s INNER JOIN node n ON s.nid = n.nid INNER JOIN users u ON n.uid = u.uid WHERE n.status = 1; 
...
SELECT a.nid, a.url, a.hostname, a.uid, a.timestamp, n.title FROM accesslog a LEFT JOIN node n ON a.nid = n.nid WHERE a.nid = 'check_query($id)';
SELECT a.nid, a.url, a.hostname, a.uid, a.timestamp, n.title FROM accesslog a LEFT JOIN node n ON a.nid = n.nid WHERE a.uid = 'check_query($id)';
...
SELECT c.*, u.name, u.uid FROM comments c INNER JOIN users u ON u.uid = c.uid WHERE c.status = '';
...
SELECT n.nid, n.title, n.type, n.changed, n.uid, u.name, MAX(n.changed, c.timestamp) AS last_activity FROM node n LEFT JOIN comments c ON n.nid = c.nid INNER JOIN users u ON n.uid = u.uid WHERE n.uid = 'check_query($id)' AND n.status = 1 GROUP BY n.nid, n.title, n.type, n.changed, n.uid, u.name;
