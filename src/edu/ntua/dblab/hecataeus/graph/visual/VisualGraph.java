...
public class VisualGraph extends DirectedSparseGraph<VisualNode, VisualEdge> implements Observer {
	...
	public VisualNode findVisualNodeertexByName(String name, NodeCategory nc) {
	...
	public boolean evolutionEdgeAdded(EvolutionEdge addedEdge) {
		VisualNode fromNode = findVisualNodeFromParent((EvolutionNode) addedEdge.getFromNode());
		VisualNode toNode = findVisualNodeFromParent((EvolutionNode) addedEdge.getToNode());
		if (fromNode == null || toNode == null) //Some VisualGraphs may not have the given Edge
			return false;

		parentEdgeMap.put(addedEdge, addedEdge.produceVisualEdge());
		VisualEdge edge = findVisualEdgeFromParent(addedEdge);
		return super.addEdge(edge, fromNode, toNode);
	}

	@Override
	public boolean evolutionVertexAdded(EvolutionNode addedNode) {
		parentNodesMap.put(addedNode, addedNode.produceVisualNode());
		return super.addVertex(findVisualNodeFromParent(addedNode));
	}

	@Override
	public boolean evolutionEdgeRemoved(EvolutionEdge removedEdge) {
		VisualEdge edge = findVisualEdgeFromParent(removedEdge);
	...
	public boolean evolutionVertexRemoved(EvolutionNode removedNode) {
		VisualNode node = findVisualNodeFromParent(removedNode);
	...
	private void addVisualEdges(List<VisualEdge> edgeList) {
		edgeList.forEach(edge -> super.addEdge(edge, edge.getFromNode(), edge.getToNode()));
	}

	private VisualNode findVisualNodeFromParent(EvolutionNode parent) {
		return parentNodesMap.get(parent);
	}

	private VisualEdge findVisualEdgeFromParent(EvolutionEdge parent) {
